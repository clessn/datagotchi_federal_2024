#' Construction du modèle RTA amélioré avec interactions régionales (6 régions) et variables de grandes villes
#' 
#' Ce script construit un modèle multinomial qui intègre les prédictions basées sur
#' les RTA (Forward Sortation Area) et ajoute des modèles distincts pour les 6 régions du Canada et 3 grandes villes:
#' 1. Ontario
#' 2. Québec
...
#'
#' Sortie :
#' - Modèle final avec prédictions RTA, interactions régionales et grandes villes (finalmodel_withRTAPredictions_Cities3_2025-04-10.rds)
#' - Variables dummy pour prédictions futures (dummies_finalmodel_withRTAPredictions_Cities3_2025-04-10.rds)
#' - Formule du modèle final (formula_finalmodel_withRTAPredictions_Cities3_2025-04-10.rds)
#'
# ------------------------------------------------------------------------
...
# ------------------------------------------------------------------------
# 6) Création des variables régionales et de grandes villes
# ------------------------------------------------------------------------
...
# Définir les codes postaux des grandes villes
# Montréal: FSA commençant par H
# Toronto: FSA commençant par M
# Vancouver: FSA commençant par V5, V6

# Ajouter des variables pour les grandes villes
DataPilot_enriched <- DataPilot_enriched %>%
  mutate(
...
# Vérifier le nombre de répondants par grande ville
city_counts <- DataModel %>%
  summarise(
    montreal = sum(is_montreal),
    toronto = sum(is_toronto),
    vancouver = sum(is_vancouver)
  )

print(city_counts)

cat("Pourcentage de répondants par grande ville:\n")
cat("Montréal:", city_counts$montreal / nrow(DataModel) * 100, "%\n")
cat("Toronto:", city_counts$toronto / nrow(DataModel) * 100, "%\n")
cat("Vancouver:", city_counts$vancouver / nrow(DataModel) * 100, "%\n")

# Distribution des votes par grande ville
city_vote_distribution <- DataModel %>%
  ...
  mutate(
    pct_montreal = montreal / sum(montreal) * 100,
    pct_toronto = toronto / sum(toronto) * 100,
    pct_vancouver = vancouver / sum(vancouver) * 100
  )

print(city_vote_distribution)
...
# Visualiser les différences régionales et urbaines dans les variables clés
...
city_differences <- DataModel %>%
  summarise(
    ...
    montreal = list(
      n = sum(is_montreal),
      avg_age = mean(as.numeric(gsub("[^0-9]", "", ses_age[is_montreal == 1])), na.rm = TRUE),
      pct_french = mean(ses_language[is_montreal == 1] == "french", na.rm = TRUE) * 100,
      pct_urban = mean(ses_dwelling_cat[is_montreal == 1] %in% c("apartment", "condo"), na.rm = TRUE) * 100
    ),
    toronto = list(
      n = sum(is_toronto),
      avg_age = mean(as.numeric(gsub("[^0-9]", "", ses_age[is_toronto == 1])), na.rm = TRUE),
      pct_french = mean(ses_language[is_toronto == 1] == "french", na.rm = TRUE) * 100,
      pct_urban = mean(ses_dwelling_cat[is_toronto == 1] %in% c("apartment", "condo"), na.rm = TRUE) * 100
    ),
    vancouver = list(
      n = sum(is_vancouver),
      avg_age = mean(as.numeric(gsub("[^0-9]", "", ses_age[is_vancouver == 1])), na.rm = TRUE),
      pct_french = mean(ses_language[is_vancouver == 1] == "french", na.rm = TRUE) * 100,
      pct_urban = mean(ses_dwelling_cat[is_vancouver == 1] %in% c("apartment", "condo"), na.rm = TRUE) * 100
    )
  )

# Extraction et affichage des résultats des grandes villes
city_stats <- data.frame(
  City = c("Overall", "Montreal", "Toronto", "Vancouver"),
  n = c(city_differences$total$n, city_differences$montreal$n, city_differences$toronto$n, city_differences$vancouver$n),
  avg_age = c(city_differences$total$avg_age, city_differences$montreal$avg_age, city_differences$toronto$avg_age, city_differences$vancouver$avg_age),
  pct_french = c(city_differences$total$pct_french, city_differences$montreal$pct_french, city_differences$toronto$pct_french, city_differences$vancouver$pct_french),
  pct_urban = c(city_differences$total$pct_urban, city_differences$montreal$pct_urban, city_differences$toronto$pct_urban, city_differences$vancouver$pct_urban)
)
...
set.seed(42)
# Stratifier par parti, région, grandes villes et source pour maintenir la distribution
trainIndex <- createDataPartition(
  interaction(DataModel$dv_voteChoice, DataModel$region_model, DataModel$is_montreal, DataModel$is_toronto, DataModel$is_vancouver, DataModel$source), 
  p = 0.8, 
  list = FALSE
)
...
cat("Distribution dans l'ensemble d'entraînement par grande ville:", "\n")
city_train_counts <- data.frame(
  City = c("Montreal", "Toronto", "Vancouver"),
  Count = c(sum(DfTrain$is_montreal), sum(DfTrain$is_toronto), sum(DfTrain$is_vancouver)),
  Percentage = c(
    sum(DfTrain$is_montreal) / nrow(DfTrain) * 100,
    sum(DfTrain$is_toronto) / nrow(DfTrain) * 100,
    sum(DfTrain$is_vancouver) / nrow(DfTrain) * 100
  )
)
print(city_train_counts)
...
# ------------------------------------------------------------------------
# 11) Construction du modèle avec variables distinctes pour régions et villes
# ------------------------------------------------------------------------
...
# Vérifier les combinaisons vides villes/parti
cat("\nVérification des combinaisons villes/parti:\n")
city_vars <- c("is_montreal", "is_toronto", "is_vancouver")
city_names <- c("Montréal", "Toronto", "Vancouver")

for(i in 1:length(city_vars)) {
  city_var <- city_vars[i]
  city_name <- city_names[i]
  
  for(p in levels(DfTrain$dv_voteChoice)) {
    count <- sum(DfTrain[[city_var]] == 1 & DfTrain$dv_voteChoice == p)
    if(count == 0) {
      cat("ATTENTION: Aucune observation pour", city_name, "et le parti", p, "\n")
    } else if(count < 10) {
      cat("ATTENTION: Seulement", count, "observations pour", city_name, "et le parti", p, "\n")
    } else {
      cat(city_name, "a", count, "observations pour le parti", p, "\n")
    }
  }
}
...
# Construire les termes d'interaction pour chaque ville
city_terms <- list()
city_vars <- c("is_montreal", "is_toronto", "is_vancouver")
for (cv in city_vars) {
  city_terms[[cv]] <- paste0(cv, ":", available_vars)
}

# Combiner tous les termes dans la formule finale
all_terms <- c(unlist(region_terms), unlist(city_terms))
...
tryCatch({
  ...
  # Reconstruire les termes d'interaction pour chaque ville avec variables réduites
  city_terms_reduced <- list()
  for (cv in city_vars) {
    city_terms_reduced[[cv]] <- paste0(cv, ":", reduced_vars)
  }
  
  # Combiner les termes réduits
  all_terms_reduced <- c(unlist(region_terms_reduced), unlist(city_terms_reduced))
...
for (i in 1:length(city_vars)) {
  ...
  city_indices <- DfTest[[city_var]] == 1
  if (sum(city_indices) > 0) {
    acc_city <- mean(pred_class[city_indices] == DfTest$dv_voteChoice[city_indices], na.rm = TRUE)
    cat("Accuracy pour", city_name, ":", acc_city, "\n")
    
    # Matrice de confusion par ville
    conf_city <- table(pred_class[city_indices], DfTest$dv_voteChoice[city_indices])
    cat("Matrice de confusion pour", city_name, ":\n")
    print(conf_city)
    
    # F1-score par parti pour cette ville
    f1_city <- sapply(parties, function(p) {
      tp <- sum(pred_class[city_indices] == p & DfTest$dv_voteChoice[city_indices] == p, na.rm = TRUE)
      fp <- sum(pred_class[city_indices] == p & DfTest$dv_voteChoice[city_indices] != p, na.rm = TRUE)
      fn <- sum(pred_class[city_indices] != p & DfTest$dv_voteChoice[city_indices] == p, na.rm = TRUE)
      precision <- ifelse(tp + fp == 0, 0, tp / (tp + fp))
      recall <- ifelse(tp + fn == 0, 0, tp / (tp + fn))
      if (is.na(precision + recall) || precision + recall == 0) {
        return(0)
    ...
    names(f1_city) <- parties
    cat("F1-scores par parti pour", city_name, ":\n")
    print(f1_city)
    cat("\n")
  }
}
...
# ------------------------------------------------------------------------
# 14) Analyse des coefficients par région et ville
# ------------------------------------------------------------------------
cat("\n--- Analyse des coefficients par région et ville ---\n")
...
tryCatch({
  ...
  if (length(important_vars) > 0) {
    # Analyse par région
    cat("\nComparaison des coefficients par région:\n")
    ...
    # Analyse par ville
    cat("\nComparaison des coefficients par grande ville:\n")
    for (var in important_vars) {
      cat("\nVariable:", var, "\n")
      ...
      for (i in 1:length(city_vars)) {
        ...
        var_name <- paste0(city_var, ":", var)
        matching_vars <- grep(var_name, all_model_vars, value = TRUE)
        
        if (length(matching_vars) > 0) {
          for (exact_var in matching_vars) {
            for (party in rownames(model_coef)) {
              cat("  Ville", city_name, "pour", party, ":", model_coef[party, exact_var], "\n")
            }
          }
        }
      }
...
tryCatch({
  ...
  model_file_name <- "_SharedFolder_datagotchi_federal_2024/data/modele/finalmodel_withRTAPredictions_Cities3_2025-04-10.rds"
  ...
  dummies_file_name <- "_SharedFolder_datagotchi_federal_2024/data/modele/dummies_finalmodel_withRTAPredictions_Cities3_2025-04-10.rds"
  saveRDS(dummies_final, dummies_file_name)
  cat("Dummies sauvegardés dans:", dummies_file_name, "\n")

  # Sauvegarder la formule finale
  formula_file_name <- "_SharedFolder_datagotchi_federal_2024/data/modele/formula_finalmodel_withRTAPredictions_Cities3_2025-04-10.rds"
  ...
  region_config_file <- "_SharedFolder_datagotchi_federal_2024/data/modele/region_mapping_Cities3_2025-04-10.rds"
  saveRDS(region_config, region_config_file)
  cat("Configuration des régions sauvegardée dans:", region_config_file, "\n")

  # Résumé des résultats
  model_results <- data.frame(
    Model = "Modèle 6 Régions + 3 Villes",
    Accuracy = acc,
  ...
  # Ajouter les résultats par ville
  for (i in 1:length(city_vars)) {
    city_var <- city_vars[i]
    city_name <- gsub(" ", "_", city_names[i])
    
    city_indices <- DfTest[[city_var]] == 1
    if (sum(city_indices) > 0) {
      acc_city <- mean(pred_class[city_indices] == DfTest$dv_voteChoice[city_indices], na.rm = TRUE)
      model_results[[paste0("Accuracy_", city_name)]] <- acc_city
    }
  }

  # Sauvegarder les résultats
  results_file_name <- "_SharedFolder_datagotchi_federal_2024/data/modele/results_model_Cities3_2025-04-10.rds"
...
cat("1. Implémentation d'un modèle avec coefficients distincts pour 6 régions et 3 grandes villes:\n")
cat("   - Ontario\n")
cat("   - Québec\n")
...
cat("3. Identification des grandes villes basée sur les codes postaux:\n")
cat("   - Montréal: codes postaux commençant par H\n")
cat("   - Toronto: codes postaux commençant par M\n")
cat("   - Vancouver: codes postaux commençant par V5 ou V6\n\n")

cat("4. Évaluation détaillée par région et par ville:\n")
cat("   - Matrices de confusion spécifiques par région et par ville\n")
cat("   - F1-scores par parti, par région et par ville\n")
cat("   - Identification des forces et faiblesses du modèle selon les régions et les villes\n\n")

cat("5. Analyse des coefficients régionaux et urbains:\n")
cat("   - Identification des facteurs qui varient entre les régions et les villes\n")
cat("   - Quantification des différences d'effets entre régions et villes\n\n")

cat("6. Améliorations techniques:\n")
cat("   - Stratification avancée pour l'échantillonnage train/test intégrant les variables de villes\n")
