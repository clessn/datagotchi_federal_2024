...
cluster_info <- data.frame(
  ...
  image_path     = rep("_SharedFolder_datagotchi_federal_2024/images/landingPage_potgrowth/ClusterRoom_Jennifer.png", 10),
  descriptionFr  = rep("_SharedFolder_datagotchi_federal_2024/images/landingPage_potgrowth/ClusterDescription_Jennifer.png"),
  descriptionEn  = rep("_SharedFolder_datagotchi_federal_2024/images/landingPage_potgrowth/ClusterDescription_Jennifer.png"),
...
for(cluster in unique(df_plot$cluster_name)) {
  ...
  # Charger l'image principale du cluster
  ...
  image_plot <- ggdraw() +
    draw_grob(img_grob, x = 0.05, y = 0.10, width = 0.8, height = 0.8)
  
  # Charger l'image de description FR
  desc_fr_path <- df_filtered$descriptionFr[1]
  if (!file.exists(desc_fr_path)) stop("L'image de description FR n'existe pas à ce chemin : ", desc_fr_path)
  desc_fr_img <- readPNG(desc_fr_path)
  desc_fr_grob <- rasterGrob(desc_fr_img, interpolate = TRUE)
  plot_textFr <- ggdraw() +
    draw_grob(desc_fr_grob, x = -0.05, y = 0, width = 1)
  
  # Charger l'image de description EN
  desc_en_path <- df_filtered$descriptionEn[1]
  if (!file.exists(desc_en_path)) stop("L'image de description EN n'existe pas à ce chemin : ", desc_en_path)
  desc_en_img <- readPNG(desc_en_path)
  desc_en_grob <- rasterGrob(desc_en_img, interpolate = TRUE)
  plot_textEn <- ggdraw() +
    draw_grob(desc_en_grob, x = 0, y = 0, width = 1)
  ...
  # -------------------- Assemblage final FR (3 colonnes + méthodologie) --------------------
  # 1) Assembler en 3 colonnes : image principale, image description, graphique RCI
  three_cols_Fr <- plot_grid(
    image_plot,
    plot_textFr,
    plot_rciFr,
    ncol = 3,
    rel_widths = c(2, 2, 3)  # 1/3 - 1/3 - 1/3
  )
  # 2) Superposer la méthodologie en bas à gauche
  final_plotFr <- ggdraw(three_cols_Fr) +
    draw_plot(
      plot_methodologyFr,
  ...
  # -------------------- Assemblage final EN (3 colonnes + méthodologie) --------------------
  three_cols_En <- plot_grid(
    image_plot,
    plot_textEn,
    plot_rciEn,
    ncol = 3,
  ...
  final_plotEn <- ggdraw(three_cols_En) +
    draw_plot(
      plot_methodologyEn,
      x = 0.005, y = -0.12,
  ...
  ggsave(
    ...
    plot = final_plotFr,
    width = 20, height = 10, dpi = 300, bg = "white", device = "png"
  )
  ggsave(
