...
df_grouped <- df_grouped %>%
  mutate(transport_label = case_when(
    ...
    lifestyle_Transport == "motorcycle" ~ "Motorcycle",
    TRUE ~ as.character(lifestyle_Transport)
  ))

# Define order for transport modes
transport_order <- c("Car", "SUV", "Public Transit", "Walking", "Bicycle", "Motorcycle")
...
transport_plot_modified <- ggplot(df_grouped, aes(x = transport_label, y = percentage, fill = dv_voteChoice)) +
  ...
  labs(
    title = "TRANSPORTATION MODES USED BY POLITICAL PARTY",
    subtitle = "Percentage of voters from each party using different transportation modes",
    x = "",
    y = "Percentage (%)",
    caption = paste0("Source: Léger-Datagotchi 2025 | n=", format(nrow(df), big.mark = " "), 
                   "\nData weighted by: gender, age, province, language, education level, income, immigration, housing type")
  ) +
...
# Read the logo
logo_path <- "_SharedFolder_datagotchi_federal_2024/logos/EN/logo_black.png"
if (file.exists(logo_path)) {
  ...
  image_write(final_img, "_SharedFolder_datagotchi_federal_2024/graph/analyses/transport/transport_vote_modified_en.png")
  
  # Remove temporary file
  file.remove(temp_file)
  
  cat("Graph successfully created with logo: transport_vote_modified_en.png\n")
} else {
  # If logo not found, save without it
  ggsave("_SharedFolder_datagotchi_federal_2024/graph/analyses/transport/transport_vote_modified_en.png", 
  ...
  cat("Graph successfully created without logo: transport_vote_modified_en.png\n")
