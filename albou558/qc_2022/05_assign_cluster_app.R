...
fct_05_process_app_data <- function(data, scale_origins, scale_ecarts_types, selected_vars = c(
    # variables_act
...
# 2) Identifier toutes les colonnes à normaliser (toutes celles sélectionnées)
numeric_cols <- selected_vars
data_numeric <- data_selected[, numeric_cols, drop = FALSE]

# 3) Normaliser en utilisant les paramètres fournis
data_scaled <- sweep(data_numeric, 2, scale_origins[numeric_cols], FUN = "-")
data_scaled <- sweep(data_scaled, 2, scale_ecarts_types[numeric_cols], FUN = "/")
...
app_data_processed <- fct_05_process_app_data(appData_clust, scale_origins, scale_ecarts_types)

app_data_clustered <- fct_05_assign_clusters(app_data_processed, centroids)
