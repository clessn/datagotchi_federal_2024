...
plot_caq <- ggplot(data_CAQ, aes(x = day, y = probability, color = cluster_name)) +
  geom_line(linewidth = 1) +
  scale_x_continuous(breaks = breaks_days, limits = c(min_day, max_day)) +
  scale_y_continuous(limits = c(0, 1)) +
  scale_color_manual(values = cluster_colors) +
  labs(
    title = "Évolution de la probabilité de vote pour la CAQ",
    x = "Jour",
    y = "Probabilité",
    color = "Cluster"
  ) +
...
ggsave(
  filename = paste0(output_dir, "/2022_evolution_CAQ.png"),
  plot = plot_caq,
  width = 12,
  height = 8
)
