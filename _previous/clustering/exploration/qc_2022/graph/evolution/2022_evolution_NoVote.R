...
plot_noVote <- ggplot(data_NoVote, aes(x = day, y = probability, color = cluster_name)) +
  geom_line(linewidth = 1) +
  scale_x_continuous(breaks = breaks_days, limits = c(min_day, max_day)) +
  scale_y_continuous(limits = c(0, 1)) +
  scale_color_manual(values = cluster_colors) +
  labs(
    title = "Évolution de la probabilité de ne pas voter",
    x = "Jour",
    y = "Probabilité",
    color = "Cluster"
  ) +
...
ggsave(
  filename = paste0(output_dir, "/2022_evolution_NoVote.png"),
  plot = plot_noVote,
  width = 12,
  height = 8
)
