...
plot_cluster5 <- ggplot(data_cluster5, aes(x = day, y = probability, color = party)) +
  geom_line(size = 1) +
  scale_color_manual(values = party_colors) +
  scale_x_continuous(breaks = breaks_days, limits = c(min_day, max_day)) +
  scale_y_continuous(limits = c(0, 1)) +
  labs(
    title = "Probabilité des partis pour le cluster 5. Isabelle - Classe moyenne Moderne",
    x = "Jour",
    y = "Probabilité",
    color = "Parti"
  ) +
...
ggsave(
  filename = paste0(output_dir, "/2022_evolution_cluster5.png"),
  plot = plot_cluster5,
  width = 12,
  height = 8
)
