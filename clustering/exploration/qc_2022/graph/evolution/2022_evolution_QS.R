...
plot_qs <- ggplot(data_QS, aes(x = day, y = probability, color = cluster_name)) +
  geom_line(linewidth = 1) +
  scale_x_continuous(breaks = breaks_days, limits = c(min_day, max_day)) +
  scale_y_continuous(limits = c(0, 1)) +
  scale_color_manual(values = cluster_colors) +
  labs(
    title = "Évolution de la probabilité de vote pour QS",
    x = "Jour",
    y = "Probabilité",
    color = "Cluster"
  ) +
...
ggsave(
  filename = paste0(output_dir, "/2022_evolution_QS.png"),
  plot = plot_qs,
  width = 12,
  height = 8
)
