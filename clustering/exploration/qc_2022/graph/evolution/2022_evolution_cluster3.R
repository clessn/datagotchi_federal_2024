...
data_cluster3 <- data_long %>% 
  filter(cluster == 3)

# Déterminer le minimum et le maximum des jours
min_day <- 0
max_day <- max(data_cluster3$day)
...
plot_cluster3 <- ggplot(data_cluster3, aes(x = day, y = probability, color = party)) +
  ...
  labs(
    title = "Probabilité des partis pour le cluster 3. James - Patriarche Anglophone",
    x = "Jour",
    y = "Probabilité",
    color = "Parti"
  ) +
...
ggsave(
  filename = paste0(output_dir, "/2022_evolution_cluster3.png"),
  plot = plot_cluster3,
  width = 12,
  height = 8
)
