...
min_day <- 0
max_day <- max(data_Conservative$day)
...
plot_cons <- ggplot(data_Conservative, aes(x = day, y = probability, color = cluster_name)) +
  geom_line(linewidth = 1) +
  scale_x_continuous(breaks = breaks_days, limits = c(min_day, max_day)) +
  scale_y_continuous(limits = c(0, 1)) +
  scale_color_manual(values = cluster_colors) +
  labs(
    title = "Évolution de la probabilité de vote pour les Conservateurs",
    x = "Jour",
    y = "Probabilité",
    color = "Cluster"
  ) +
...
ggsave(
  filename = paste0(output_dir, "/2021_evolution_Cons.png"),
  plot = plot_cons,
  width = 10,
  height = 6
)
